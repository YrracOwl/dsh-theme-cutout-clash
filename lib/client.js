...
body[data-cutout-clash] [data-queue-dock] > div { border:2px solid var(--cc-ink); border-bottom:0; border-radius:4px 4px 0 0; box-shadow:3px 0 0 var(--cc-shadow); background:var(--dsw-alias-bg-layer-1); color:var(--dsw-alias-label-primary); }
body[data-cutout-clash] [data-queue-dock] > div > button { color:var(--dsw-alias-label-primary); font-weight:700; }
/* Composer attachment button: the component paints it with
   --dsw-specific-selector. Match it by its stable aria-label so the glyph
   survives whatever surface that token carries. */
body[data-cutout-clash] button[aria-label][class*="_add"] { color:var(--dsw-alias-label-primary); }
body[data-cutout-clash] button[aria-label][class*="_add"]:hover:not(:disabled) { color:var(--dsw-alias-bg-layer-1); background:var(--cc-ink); }
